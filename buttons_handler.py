...
def extract_telegram_deep_link(link_text: str, bot_username: str) -> Optional[str]:
    ...
    USA GUIÓN (-) como separador del message_id para evitar conflicto con usernames que tienen _
    
    t.me/c/1234567890/123 → bot?start=c_1234567890-123
    t.me/just_clinicase/30 → bot?start=p_justclinicase-30
    """
    ...
    if private_id:
        # Canal privado: c_CHANNELID-MSGID
        return f"https://t.me/{bot_username}?start=c_{private_id}-{message_id}"
    elif public_username:
        # Canal público: p_USERNAME-MSGID (guión antes del ID)
        # Quitar guiones bajos del username para el deep link
        clean_username = public_username.replace('_', '')
        return f"https://t.me/{bot_username}?start=p_{public_username}-{message_id}"
